...
export const TakePhoto = function (callback, err, photoRem, canRemovePhoto) {
  let opt = options;
  if (canRemovePhoto) {
    opt = {
      ...options,
...
export const openGallery = function (callback) {
  ImagePickerCustomized.openPicker(multiplePickerOptions)
    .then((response) => {
      callback(response);
    })
    .catch((err) => {});
...
export const openCamMultiple = function (callback) {
  ImagePickerCustomized.openCamera({
